...
def chunk_pages(text, chunk_chars=2000):
    chunks = []
    start = 0
    page = 1
...
def map_to_kingwen_states(upgrade_categories, findings):
    hex_ids = []
    phase_suggestions = []
    for category in upgrade_categories:
        for hid, mapped in HEXAGRAM_UPGRADE_MAP.items():
            if mapped == category and hid not in hex_ids:
                hex_ids.append(hid)
                combined = ' '.join(f['sentence'] for f in findings).lower()
                if any(k in combined for k in ['future', 'emerg', 'novel', 'new', 'next']):
                    phase_suggestions.append({"hexagram_id": hid, "phase_bits": 2, "phase_temporal": "future"})
                elif any(k in combined for k in ['current', 'present', 'state-of-the-art', 'baseline', 'existing']):
                    phase_suggestions.append({"hexagram_id": hid, "phase_bits": 1, "phase_temporal": "present"})
                else:
                    phase_suggestions.append({"hexagram_id": hid, "phase_bits": 0, "phase_temporal": "past"})
                break
    return hex_ids[:12], phase_suggestions[:12]
...
for idx, paper in enumerate(manifest, 1):
    ...
    for chunk in chunks:
        ...
        page_hex = []
        for hid, mapped in HEXAGRAM_UPGRADE_MAP.items():
            if mapped in page_cats and hid not in page_hex:
                page_hex.append(hid)
        per_page.append({
            ...
            "findings": findings,
            "kingwen_hexagram_ids": page_hex[:6],
            "kingwen_phase_suggestions": [
                {"hexagram_id": hid, "phase_bits": 0, "phase_temporal": "past"}
                for hid in page_hex[:6]
            ]
        })
    
    hex_ids, phase_suggestions = map_to_kingwen_states(upgrade_categories, all_findings)
